...
def classify_sentences(model_dir, base_model, data_path, output_path, text_column="sentence",
    ...
    if minimal_output:
        df[[text_column, "pseudo_label"]].rename(columns={"pseudo_label": "env_label"}).to_csv(output_path, index=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Classify sentences with a fine-tuned Environmental model for pseudo-labeling.")
    parser.add_argument("--model_dir", type=str, default="../finetuned_cv_models/env_distilroberta_fold4_best",
                        help="Path to the fine-tuned model to use.")
    parser.add_argument("--base_model", type=str, default="distilroberta-base",
                        help="Name of the base model used during training.")
    parser.add_argument("--data_path", type=str, default="../../data/pseudo_labeled/base_data_E_pseudo.csv",
                        help="Path to the CSV file to classify.")
    parser.add_argument("--output_path", type=str, default="../../data/pseudo_labeled/base_data_E_pseudo.csv",
    ...
    parser.add_argument("--threshold", type=float, default=0.0,
                        help="Minimum probability required to retain a pseudo-label (0.0 = no filtering).")
    parser.add_argument("--no_source_column", action="store_true",
                        help="Do not add a 'label_source' column.")
    parser.add_argument("--minimal_output", action="store_true",
                        help="Only save the 'sentence' and 'env_label' columns.")
