...
def classify_sentences(model_dir, base_model, data_path, output_path, text_column="sentence",
    ...
    if threshold > 0.0:
        ...
        print(f"📉 Filtered with threshold={threshold}: {len(df)} rows retained.")

    if minimal_output:
        df[[text_column, "pseudo_label"]].rename(columns={"pseudo_label": "gov_label"}).to_csv(output_path, index=False)
        print(f"\n✅ Done! Saved in minimal CSV format to: {output_path}")
    else:
        if add_source_column:
            df["label_source"] = "pseudo"
        df.rename(columns={"pseudo_label": "gov_label"}).to_csv(output_path, index=False)
        print(f"\n✅ Done! Full classification saved to: {output_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Classify sentences using a fine-tuned Governance model.")
    parser.add_argument("--model_dir", type=str, default="../finetuned_cv_models/gov_roberta_cv/checkpoint-100",
    ...
    parser.add_argument("--data_path", type=str, default="../data/base_data_train_G.csv",
                        help="Path to the CSV file to be classified.")
    parser.add_argument("--output_path", type=str, default="../data/base_data_train_G-classified.csv",
    ...
    parser.add_argument("--minimal_output", action="store_true",
                        help="Only save the 'sentence' and 'gov_label' columns.")
